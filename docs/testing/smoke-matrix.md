...
| AI provider/agent menu gate | yes | via basic | `GET /api/admin/v1/users/me` menu/router payload | no | n/a | users/me must not return retired AI menu entries `/ai/goods`, `/ai/cine`, `/ai/models`, or `/ai/agent`; it must return the nine active AI entries `/ai/providers`, `/ai/agents`, `/ai/knowledge`, `/ai/tools`, `/ai/runs`, `/ai/chat`, `/ai/image-playground`, `/ai/prompts`, and `/ai/assets`. Smoke clears current-user button cache before this gate so stale Redis grants do not hide DB/menu truth. |
