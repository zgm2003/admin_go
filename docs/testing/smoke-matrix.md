...
| AI provider/agent/map config read | no | yes | `GET /api/admin/v1/ai-providers/page-init`, `GET /api/admin/v1/ai-providers`, `GET /api/admin/v1/ai-agents/page-init`, `GET /api/admin/v1/ai-agents`, `GET /api/admin/v1/ai-agents?scene=chat`, `GET /api/admin/v1/ai-agents/options`, `GET /api/admin/v1/ai-knowledge-maps/page-init`, `GET /api/admin/v1/ai-knowledge-maps`, `GET /api/admin/v1/ai-tool-maps/page-init`, `GET /api/admin/v1/ai-tool-maps` | no | n/a | full smoke proves the provider/agent admin tables are Go-owned, list/init shapes are stable, provider driver is exactly `openai`, health/model-sync statuses are `unknown` / `ok` / `failed`, agent init exposes `scene_arr` and `provider_model_options`, agent list exposes `model_id` / `scenes` / `system_prompt` / `avatar`, scene search is wired, and secret/raw/source/config fields such as raw API keys, encrypted key blobs, remote model raw payloads, source markers, provider config JSON blobs, agent code/type, per-agent external app/key fields, response mode, runtime config, or model snapshot JSON do not leak. It must not call old AI model/tool agent-options/prompt/agent/knowledge-base REST resources. |
| AI chat/runtime/runs disabled-baseline | no | yes | `GET /api/admin/v1/ai-conversations`, `GET /api/admin/v1/ai-runs/page-init`, `GET /api/admin/v1/ai-runs`, `GET /api/admin/v1/ai-runs/stats`, `POST /api/admin/v1/ai-chat/runs` disabled-baseline failure probe | no successful mutation by default | n/a | full smoke proves conversation/run read paths use the local AI schema and that chat without an enabled provider/agent returns explicit `code=100` config failure such as `请先配置 AI 供应商和智能体`; optional live provider probe is gated by `-EnableAiProviderProbe` plus `AI_SMOKE_AGENT_ID` and is not required for default CI. |
...
go test ./internal/platform/ai ./internal/platform/ai/provider ./internal/module/aiprovider ./internal/module/aiagent ./internal/module/aiknowledgemap ./internal/module/aitoolmap ./internal/module/aichat ./internal/module/aiconversation ./internal/module/aimessage ./internal/module/airun ./internal/server ./internal/bootstrap
```
