...
| AI sidecar menu gate | yes | via basic | `GET /api/admin/v1/users/init` menu/router payload | no | n/a | users/init must not return retired goods/cine/model/agent/prompt AI menu entries; it must return the six active AI entries `/ai/providers`, `/ai/apps`, `/ai/chat`, `/ai/knowledge`, `/ai/runs`, and `/ai/tools`. Smoke clears current-user button cache before this gate so stale Redis grants do not hide DB/menu truth. |
| AI provider/app/map config read | no | yes | `GET /api/admin/v1/ai-engine-connections/page-init`, `GET /api/admin/v1/ai-engine-connections`, `GET /api/admin/v1/ai-apps/page-init`, `GET /api/admin/v1/ai-apps`, `GET /api/admin/v1/ai-apps/options`, `GET /api/admin/v1/ai-knowledge-maps/page-init`, `GET /api/admin/v1/ai-knowledge-maps`, `GET /api/admin/v1/ai-tool-maps/page-init`, `GET /api/admin/v1/ai-tool-maps` | no | n/a | full smoke proves the Dify sidecar admin tables are Go-owned, list/init shapes are stable, and secret fields such as raw API keys or encrypted key blobs do not leak. It must not call old AI model/tool agent-options/prompt/agent/knowledge-base REST resources. |
| AI chat/runtime/runs disabled-baseline | no | yes | `GET /api/admin/v1/ai-conversations`, `GET /api/admin/v1/ai-runs/page-init`, `GET /api/admin/v1/ai-runs`, `GET /api/admin/v1/ai-runs/stats`, `POST /api/admin/v1/ai-chat/runs` disabled-baseline failure probe | no successful mutation by default | n/a | full smoke proves conversation/run read paths use the sidecar schema and that chat without an enabled app/engine returns explicit `code=100` engine-config failure such as `请先配置 AI 应用和 Dify 连接`; optional live Dify probe is gated by explicit smoke variables and is not required for default CI. |
...
go test ./internal/platform/ai ./internal/platform/ai/dify ./internal/module/aiengine ./internal/module/aiapp ./internal/module/aiknowledgemap ./internal/module/aitoolmap ./internal/module/aichat ./internal/module/aiconversation ./internal/module/aimessage ./internal/module/airun ./internal/server ./internal/bootstrap
```
