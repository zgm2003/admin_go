...
| system cron tasks | no | yes | `GET /api/admin/v1/cron-tasks/init`, `GET /api/admin/v1/cron-tasks`, conditional `GET /api/admin/v1/cron-tasks/:id/logs` | no | n/a | full smoke probes dict/page/list/log shape and asserts `notification_task_scheduler`, `pay_close_expired_order`, and `pay_sync_pending_transaction` are `registered` with Go task type handlers; legacy PHP-only rows remain visible as `missing` instead of fake-executed |
...
| chat room first slice | no | unit/type/contract/build only | `GET/POST/PATCH/DELETE /api/admin/v1/chat...`, `chat.message.created.v1`, `chat.read.v1`, local Element Plus X `XSender`/`Conversations`/`BubbleList`/`Bubble` subpath imports, MessageInput composition guard, ConversationList guard, MessageList Bubble guard, and `chat-ui` bundle group | no in smoke | n/a | 当前只用 `go test ./internal/module/chat ./internal/server ./internal/bootstrap`、前端 chat contract Vitest、MessageInput Element Plus X/composition guard tests、ConversationList Element Plus X guard test、MessageList Element Plus X guard test、targeted eslint、`vue-tsc` 和 `build:analyze` 验证；不在 smoke 里发送真实聊天消息或污染联系人/会话 fixture；后续需要专门 chat e2e fixture 再进 full smoke |
