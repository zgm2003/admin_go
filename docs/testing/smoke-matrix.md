...
| system cron tasks | no | yes | `GET /api/admin/v1/cron-tasks/init`, `GET /api/admin/v1/cron-tasks`, conditional `GET /api/admin/v1/cron-tasks/:id/logs` | no | n/a | full smoke probes dict/page/list/log shape and asserts `notification_task_scheduler` is `registered`; legacy PHP-only rows remain visible as `missing` instead of fake-executed |
...
| scheduler business dispatch | no | unit tests | `cron_task.name=notification_task_scheduler` -> `notification:dispatch-due:v1` -> `notification:send-task:v1` | no in smoke | n/a | 调度器不放进 smoke 写路径；用 `go test ./internal/module/crontask ./internal/module/notificationtask ./internal/jobs ./internal/bootstrap` 证明 DB-backed scheduler 只写 cron_task_log 并 enqueue，handler 才 claim DB/send |
