...
| wallet admin read | no | yes | `GET /api/admin/v1/wallets/page-init`, `GET /api/admin/v1/wallets`, `GET /api/admin/v1/wallet-transactions` | no | n/a | full smoke 只探测后台钱包字典、钱包分页、钱包流水分页 shape；`wallet_transactions` 可为空；不执行调账、不改余额、不触发支付 SDK |
