...
| system cron tasks | no | yes | `GET /api/admin/v1/cron-tasks/init`, `GET /api/admin/v1/cron-tasks`, conditional `GET /api/admin/v1/cron-tasks/:id/logs` | no | n/a | full smoke probes dict/page/list/log shape and asserts `notification_task_scheduler`, `ai_run_timeout`, `payment_sync_pending_order`, and `payment_close_expired_order` are `registered` with Go task type handlers; payment tasks must report `payment:sync-pending-order:v1` / `payment:close-expired-order:v1`; `ai_run_timeout` must report `ai:run-timeout:v1`. Smoke checks registry/list shape and does not intentionally kill fresh AI replies or execute payment compensation writes. |
