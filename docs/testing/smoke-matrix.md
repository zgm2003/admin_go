...
| system cron tasks | no | yes | `GET /api/admin/v1/cron-tasks/init`, `GET /api/admin/v1/cron-tasks`, conditional `GET /api/admin/v1/cron-tasks/:id/logs` | no | n/a | full smoke probes dict/page/list/log shape and asserts `notification_task_scheduler`, `pay_close_expired_order`, and `pay_sync_pending_transaction` are `registered` with Go task type handlers; `pay_fulfillment_retry`, `pay_reconcile_daily`, and `pay_reconcile_execute` remain `missing` until their Go handlers land; `pay_refund_sync` is a deleted legacy row unless a refund spec enables it |
