...
| system cron tasks | no | yes | `GET /api/admin/v1/cron-tasks/init`, `GET /api/admin/v1/cron-tasks`, conditional `GET /api/admin/v1/cron-tasks/:id/logs` | no | n/a | full smoke probes dict/page/list/log shape and asserts `notification_task_scheduler` and `ai_run_timeout` are `registered` with Go task type handlers; `ai_run_timeout` must report `ai:run-timeout:v1`; payment order cron rows must not be `registered` in the payment order Alipay pay v1 slice. `ai_run_timeout` is a stale-run sweeper only; smoke checks registry/list shape and does not intentionally kill a fresh running AI reply |
...
| upload token shape | no | gated yes | `POST /api/admin/v1/upload-tokens` | token only | n/a | 没有 enabled upload setting 时 summary 输出 `skipped_upload_setting_missing`；存在 enabled COS setting 时只校验 provider/key/credentials shape，永远不上传真实文件 |
| payment config + recharge cashier Alipay v1 | no | yes | full smoke probes `GET /api/admin/v1/payment/configs/page-init`, `GET /api/admin/v1/payment/configs?current_page=1&page_size=20`, `GET /api/admin/v1/payment/recharges/page-init`, `GET /api/admin/v1/payment/recharges?current_page=1&page_size=10`, plus low-level `GET /api/admin/v1/payment/orders/page-init`, `GET /api/admin/v1/payment/orders?current_page=1&page_size=20`; users/init menu gate asserts `/payment/config` with `view_key=payment/config` and `/payment/recharge` with `view_key=payment/recharge` are visible, `/payment/orders` is hidden/internal when present, and retired channel/event/pay/wallet menus are absent | no default mutation | n/a | Alipay config/recharge/order read only; smoke expects provider=`alipay`, asserts config secrets never leak, checks package/wallet/recharge/order list shape, and checks no raw payment create UX assumptions. Default smoke does not upload certificates, call `configs/:id/test`, create recharges/orders, trigger pay, sync real Alipay, write wallet credit, or call notify. Sandbox recharge/pay/sync is credential-gated manual smoke. |
...
Payment smoke is read-only by default in this rebuild. It probes payment config page-init/list, payment recharge page-init/list shape, low-level payment order page-init/list shape, and menu state only. Certificate upload, config test with real credentials, sandbox recharge/order creation, pay URL generation, status sync, wallet credit, notify, and real Alipay calls remain manual or future credential-gated probes.
