...
| pay reconcile admin/runtime | no | planned read-only probe | Go REST `GET /api/admin/v1/pay-reconcile-tasks/page-init`, list/detail/retry/file endpoints are implemented; Go worker registers `pay_reconcile_daily -> pay:reconcile-daily:v1` and `pay_reconcile_execute -> pay:reconcile-execute:v1`; execute currently writes local CSV then fails loudly because platform bill download is not implemented | no in current smoke | n/a | 前端 `src/api/pay/reconcile.ts` 已走 Go REST、无 legacyRequest；smoke 仍需补 read-only list/detail probe 和 cron registry gate；不能把平台账单下载/成功差异比对写成已完成。 |
