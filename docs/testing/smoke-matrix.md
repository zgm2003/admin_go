...
| payment config + order Alipay v1 | no | yes | full smoke probes `GET /api/admin/v1/payment/configs/page-init`, `GET /api/admin/v1/payment/configs?current_page=1&page_size=20`, `GET /api/admin/v1/payment/orders/page-init`, `GET /api/admin/v1/payment/orders?current_page=1&page_size=20`; users/init menu gate asserts `/payment/config` with `view_key=payment/config` and `/payment/orders` with `view_key=payment/orders` are present, and retired channel/event/pay/wallet menus are absent | no default mutation | n/a | Alipay config/order only; smoke expects provider=`alipay`, asserts config secrets never leak, and checks order list shape when rows exist. Default smoke does not upload certificates, call `configs/:id/test`, create orders, trigger pay, sync real Alipay, or write paid state. Sandbox create/pay/sync is credential-gated manual smoke. |
...
Payment smoke is read-only by default in this rebuild. It probes payment config page-init/list, payment order page-init/list shape, and menu state only. Certificate upload, config test with real credentials, sandbox order creation, pay URL generation, status sync, notify, and real Alipay calls remain manual or future credential-gated probes.
