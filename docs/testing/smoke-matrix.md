...
| AI provider/agent menu gate | yes | via basic | `GET /api/admin/v1/users/init` menu/router payload | no | n/a | users/init must not return retired goods/cine/model/agent/prompt AI menu entries; it must return the seven active AI entries `/ai/providers`, `/ai/agents`, `/ai/knowledge`, `/ai/tools`, `/ai/runs`, `/ai/chat`, and `/ai/images`. Smoke clears current-user button cache before this gate so stale Redis grants do not hide DB/menu truth. |
...
| payment config rebuild v1 | no | yes | full smoke probes `GET /api/admin/v1/payment/configs/page-init`, `GET /api/admin/v1/payment/configs?current_page=1&page_size=20`; users/init menu gate asserts `/payment/config` with `view_key=payment/config` is present and `/payment/channel`, `/payment/order`, `/payment/event`, `/pay`, `/wallet` are absent | no default mutation | n/a | Alipay config only; smoke must assert `app_private_key_enc`, plaintext private key, provider/merchant_id/sign_type/extra_config never leak; default smoke does not upload certificates, call `configs/:id/test`, create orders, or call Alipay |
...
Payment smoke is read-only by default in this rebuild. It probes payment config page-init/list and menu state only; certificate upload, config test with real credentials, order creation, notify, and real Alipay calls remain manual or future credential-gated probes.
