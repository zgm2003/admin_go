...
| system cron tasks | no | yes | `GET /api/admin/v1/cron-tasks/init`, `GET /api/admin/v1/cron-tasks`, conditional `GET /api/admin/v1/cron-tasks/:id/logs` | no | n/a | full smoke probes dict/page/list/log shape and asserts `notification_task_scheduler`, `pay_close_expired_order`, `pay_sync_pending_transaction`, `pay_reconcile_daily`, `pay_reconcile_execute`, and `pay_fulfillment_retry` are `registered` with Go task type handlers; `pay_refund_sync` is a deleted legacy row unless a refund spec enables it |
...
| pay reconcile admin/runtime | no | planned read-only probe | Go REST `GET /api/admin/v1/pay-reconcile-tasks/page-init`, list/detail/retry/file endpoints are implemented; Go worker registers `pay_reconcile_daily -> pay:reconcile-daily:v1` and `pay_reconcile_execute -> pay:reconcile-execute:v1`; execute now downloads Alipay trade bill via Go payment gateway, parses UTF-8/GBK CSV or zip content, writes `local/platform/diff` CSV, and marks success/diff/failed explicitly | no in current smoke | n/a | 前端 `src/api/pay/reconcile.ts` 已走 Go REST、无 legacyRequest；smoke 仍需补 read-only list/detail probe 和 cron registry gate；真实支付宝平台账单下载不进默认 smoke，不能把 fake 单测说成真实 sandbox E2E。 |
