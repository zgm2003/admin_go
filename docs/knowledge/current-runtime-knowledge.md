...
`backend-capability-runbook-2026-06-08.md` turns those `37` capabilities into a per-capability handoff table with source owner files, route surfaces, frontend-call counts, DB tables, direct tests, and first verification command. Use it before changing Go behavior; it does not replace source inspection.
