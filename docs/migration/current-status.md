...
| system logs | implemented baseline: slog stdout + optional lumberjack file output, read-only logstore, REST files/lines API with path traversal guard | adapted to Go REST API | `internal/platform/logstore`, `internal/platform/logging`, `internal/module/systemlog`, `internal/server` | full smoke probes init/files shape and conditionally probes lines when a log file exists | architecture + contract + plan + smoke matrix | first phase is read-only; no ELK/Loki, no delete/clear/download |
