...
- Canvas Front Next auth/RBAC shell hardening：Canvas login `data.user` 与 `/api/canvas/v1/users/me` 已收口为 users/init 同形 contract（`user_id`、`username`、`avatar`、`role_name`、`permissions`、`router`、`buttonCodes`、`quick_entry`），不返回 `display_name` / `avatar_url` / `permissionCodes` / `button_codes` 等自造 alias；Canvas SQL seed 包含 PAGE rows（`canvas_page`、`canvas_image_page`、`canvas_video_page`、`canvas_prompts_page`、`canvas_assets_page`、`canvas_profile_page`、`canvas_wallet_page`）以及 BUTTON rows（`canvas_access`、`canvas_prompt_read`、`canvas_asset_read`、`canvas_ai_image_generate`、`canvas_ai_video_generate`、`canvas_wallet_read`、`canvas_recharge_add`、`canvas_recharge_pay`）；top/mobile navigation 和 route guard 使用 backend `router`，按钮/动作 `can(code)` 只读 `buttonCodes`；login route uses an isolated shell, password captcha is an Ant Design dialog, and email/phone code input no longer uses deprecated `addonAfter`.
...
2026-05-31 Canvas Front Next auth/RBAC shell hardening verified: Canvas PAGE/BUTTON permission SQL seed, backend canvas login/current-user users/init-shaped payload (`permissions`/`router`/BUTTON-only `buttonCodes`), frontend route-path session store + BUTTON `can()`, guarded navigation/routes/API calls, isolated login shell, dialog captcha, account profile/wallet entries, focused tests, typecheck, Next build, and root governance gates passed
