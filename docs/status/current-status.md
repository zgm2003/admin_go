...
- Canvas profile/wallet/recharge parity：Canvas profile 由 `internal/module/profile/transport/canvas` 拥有 `GET/PUT /api/canvas/v1/profile`，`canvas_front_next` 的 `/profile` 使用 profile service，`/wallet` 使用独立 wallet API adapter，`/recharge` 为账号菜单入口并由 backend `router` + `canvas_recharge_*` button codes 控制。
