...
- Canvas free-generation active slice contract：`canvas_front_next` 独立 Next.js 前端只调用 `/api/canvas/v1/*`；本切片明确退休旧 AI billing 全局系统，Canvas 文本、图片、视频生成统一通过后端托管 provider 免费执行，不查余额、不扣款、不写 `ai_billing_records`。backend/frontend/root gates 和本地 live DB migration/query 已验证。
