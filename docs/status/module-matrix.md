...
- Frontend:
  ...
  - implemented: audio is now a frontend Canvas resource-reference kind for labels (`音频1`), `inputOrder`, config composer token replacement, `NodeGenerationContext.referenceAudios/audioCount`, config input summary, default specs, basic audio node rendering, storage-backed Audio node hydration, toolbar/connection-menu Audio node creation, local audio upload/drop/replacement to Audio nodes, empty Audio node hover upload, audio download, audio prompt/config mode routing, Canvas config modal default audio scene selection, frontend-only Audio prompt/config settings (`audioVoice` / `audioFormat` / `audioSpeed` / `audioInstructions`), and `agents.audio` / `audioModel` model picking. Local media storage records `durationMs` for audio uploads and preserves video dimensions while adding video duration when metadata is readable; metadata read failure does not block storing the blob. Backend/Admin scene plumbing now includes `canvas_audio_generate`; initial generation and retry both still fail closed because no `/api/canvas/v1/ai/audios` route or `ai_assets` audio contract is implemented.
