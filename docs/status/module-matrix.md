...
- Risk:
  ...
  - video advanced params C2-A is only `generate_audio` / `watermark` request pass-through; reference video/audio upload, broader Seedance parameter policy, Admin strategy UI, and live provider success remain separate slices.
  ...
  - current-canvas merge import and asset ZIP import fail-closed remain partial slices; browser/manual Cine Make flow, cloud-synced projects, asset persistence/ownership remap, audio asset backend type, explicit DB metadata columns, and broader素材/导入产品化 hardening remain unclaimed until separately tested and documented.
