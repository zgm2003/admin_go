...
- Frontend:
  ...
  - implemented: audio is now a frontend Canvas resource-reference kind for labels (`音频1`), `inputOrder`, config composer token replacement, `NodeGenerationContext.referenceAudios/audioCount`, config input summary, default specs, basic audio node rendering, storage-backed Audio node hydration, toolbar/connection-menu Audio node creation, local audio upload/drop/replacement to Audio nodes, empty Audio node hover upload, audio download, audio prompt/config mode routing, Canvas config modal default audio scene selection and default audio params, frontend-only Audio prompt/config settings (`audioVoice` / `audioFormat` / `audioSpeed` / `audioInstructions`), and `agents.audio` / `audioModel` model picking. Local media storage records `durationMs` for audio uploads and preserves video dimensions while adding video duration when metadata is readable; metadata read failure does not block storing the blob. Backend/Admin scene plumbing now includes `canvas_audio_generate`; initial generation and retry both still fail closed because no `/api/canvas/v1/ai/audios` route or `ai_assets` audio contract is implemented.
...
- Tests:
  ...
  - 2026-06-09 audio toolbar/upload/scene/settings plumbing covered by targeted Canvas Vitest (`src/components/audio-settings-panel.test.tsx`, `src/stores/use-config-store.test.ts`, `tests/shared/canvas-audio-node-scene-wiring.test.ts`, `tests/shared/canvas-audio-upload-wiring.test.ts`, `tests/shared/canvas-audio-settings-ui.test.ts`), focused Go tests (`./internal/module/canvas`, `./internal/module/canvas/transport/canvas`, `./internal/module/ai/agent`), Admin Vue targeted Vitest (`tests/shared/ai/ai-agent-api.test.ts`, `tests/shared/ai/ai-agent-free-canvas-scenes.test.ts`), and Canvas/Admin typecheck.
...
- Risk:
  ...
  - audio resource-reference and scene plumbing is not a completed audio-generation product path; current Canvas audio generation fails closed until a backend `/api/canvas/v1/ai/audios` route, provider contract, and `ai_assets` audio persistence rules are defined and verified.
