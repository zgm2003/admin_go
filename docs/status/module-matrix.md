...
- Risk:
  ...
  - audio generation has a backend route and frontend wiring, but it is not a full audio product path yet: no `ai_assets` audio type, no dedicated audio task table/history UI, no reference-media upload to provider-accessible storage, and no live provider-success smoke has been claimed.
