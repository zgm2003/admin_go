...
- Backend:
  ...
  - list supports scene search for `chat`, `agent_generate`, `canvas_text_generate`, `canvas_image_generate`, `canvas_video_generate`, and `canvas_audio_generate`; the retired non-Canvas image scene is rejected
...
- Frontend:
  - adapted: `src/api/ai/agents.ts` uses Go REST only, page route is `/ai/agents`, search supports
    ...
    and includes `工具生成` plus Canvas text/image/video/audio scene options, status select, system prompt textarea, `UpMedia` avatar, tool configuration, and
...
- Backend:
  ...
  - Canvas image edit now forwards input/mask multipart file parts with their actual `image/*` Content-Type so upstream image edit APIs do not see reference images as `application/octet-stream`.
  - Canvas image edit/reference reads now allow 60s for backend COS object downloads, after live failed tasks showed `读取图片文件失败` / AI run timeout while the persisted COS reference URLs were readable.
- Frontend:
...
- Backend:
  - implemented: `auth_platforms.canvas` seed and canvas capability permissions, `/api/canvas/v1/auth/*`, `/api/canvas/v1/users/me`, `/api/canvas/v1/profile`, public settings, AI-owned prompts/assets transports, and AI text/image/video/audio generation routes. Old Canvas wallet/recharge UI/routes are retired from the Canvas free-generation surface; payment/wallet基础域 remains outside Canvas AI.
  - route ownership: auth has dedicated `transport/admin`, `transport/app`, and `transport/canvas`; user app owns `/api/app/v1/users/me`, user canvas owns `/api/canvas/v1/users/me`; profile app owns `/api/app/v1/profile`, profile canvas owns `/api/canvas/v1/profile`; Canvas chat/video/audio AI routes are owned by `internal/module/ai/chat|video|audio/transport/canvas`, Canvas image routes are owned by `internal/module/ai/image/transport/canvas`, and Canvas prompt/asset routes are owned by `internal/module/ai/prompt|asset/transport/canvas`; payment/wallet admin routes remain payment-owned, and Canvas AI generation no longer depends on payment/wallet transport.
  ...
  - implemented: text/image/video/audio generation use backend-managed provider config for free; old `ai_billing_records(platform=canvas)` charge/refund/audit is deleted; image tasks use `ai_image_tasks/ai_image_files` with `platform=canvas`; video binds upstream task id to `canvas_video_tasks.provider_task_id`, stores the AI run binding in `canvas_video_tasks.run_id`, and reads status/content by task ownership (`id + user_id + is_del=2`); audio currently returns a raw `audio/*` blob and records only an `ai_runs` provider attempt.
  ...
  - implemented C2 reference-media storage first slice: `POST /api/canvas/v1/ai/videos/reference-media` is owned by `internal/module/ai/video/transport/canvas`, accepts multipart `file` plus `media_kind=image|video|audio`, rejects browser `model/provider/api_key/base_url`, stores through backend-owned COS config under `ai-video-references/{kind}/...`, and returns provider-accessible storage metadata. It does not wire references into `/api/canvas/v1/ai/videos` generation payloads, does not create `ai_assets`, and does not restore legacy `/api/v1/media/references`.
  - implemented: `/api/canvas/v1/settings` exposes selectable AI runtime through `agents.text|image|video|audio` derived from enabled `ai_agents` Canvas-specific scene bindings (`canvas_text_generate`, `canvas_image_generate`, `canvas_video_generate`, `canvas_audio_generate`); old billing-scene metadata is removed and must not be treated as a model source.
...
- Frontend:
  ...
  - implemented boundary: auth/settings/prompts/assets/profile/text/image/video/audio clients call `/api/canvas/v1/*`; prompt and asset clients are backed by AI module transports; no user-entered provider API key/base URL is used for generation; wallet/recharge UI is removed from Canvas free-generation surface while payment/wallet基础域保留 outside Canvas AI.
  - implemented: Canvas model/agent pickers use only configured `agents.*` options from settings, submit only `agent_id`, and no longer invent `canvas_text_generate` / `canvas_image_generate` / `canvas_video_generate` / `canvas_audio_generate` as selectable models.
  ...
  - implemented C2 reference-media helper boundary: `uploadVideoReferenceMedia()` uploads `File + media_kind` to `/api/canvas/v1/ai/videos/reference-media` and never appends browser `provider`, `model`, `api_key`, or `base_url`. Existing `referenceVideos` / `referenceAudios` generation calls still fail closed until provider reference payload wiring is separately specified.
  ...
  - implemented C5 first slice: audio is now a frontend Canvas resource-reference kind for labels (`音频1`), `inputOrder`, config composer token replacement, `NodeGenerationContext.referenceAudios/audioCount`, config input summary, default specs, basic audio node rendering, storage-backed Audio node hydration, toolbar/connection-menu Audio node creation, local audio upload/drop/replacement to Audio nodes, empty Audio node hover upload, audio download, audio prompt/config mode routing, Canvas config modal default audio scene selection and default audio params, Audio prompt/config settings (`audioVoice` / `audioFormat` / `audioSpeed` / `audioInstructions`), and `agents.audio` / `audioModel` model picking. Initial generation and retry now call `/api/canvas/v1/ai/audios`, upload the returned raw `audio/*` blob into local Canvas media storage, and update the Audio node metadata. Backend/Admin scene plumbing includes `canvas_audio_generate`; the backend route resolves provider/model/API key from that agent, rejects browser `model/provider/api_key/base_url`, and records `ai_runs`. This still does not add `ai_assets` audio type, a dedicated audio task table/history, or reference-media upload.
  ...
  - partially implemented C4-A: Canvas asset ZIP import/export in `canvas_front_next` now fails closed on malformed `assets.json`, invalid image/video metadata, storageKey/mime/bytes mismatch, duplicate exported storageKey/path, missing declared blobs, ZIP entry size mismatch, orphan file entries, and media assets whose blobs cannot be packaged. The `/assets` page backend-backed import path currently accepts text-only ZIP packages and rejects media packages before local blob writes, because image/video ZIP media still need a storage-backed upload/remap contract before they can be persisted through `/api/canvas/v1/assets`. This is frontend parser/export/import contract hardening only; audio backend asset type remains a future slice.
- Tests:
  ...
  - 2026-06-09 local media audio/video duration metadata covered by targeted Canvas Vitest (`src/services/file-storage.test.ts`) and `npm run typecheck`.
  - 2026-06-09 audio toolbar/upload/scene/settings/API plumbing covered by targeted Canvas Vitest (`src/services/api/audio.test.ts`, `src/components/audio-settings-panel.test.tsx`, `src/stores/use-config-store.test.ts`, `tests/shared/canvas-audio-node-scene-wiring.test.ts`, `tests/shared/canvas-audio-upload-wiring.test.ts`, `tests/shared/canvas-audio-settings-ui.test.ts`), focused Go tests (`./internal/module/ai/audio`, `./internal/module/ai/audio/transport/canvas`, `./internal/module/canvas`, `./internal/module/canvas/transport/canvas`, `./internal/module/ai/agent`, `./internal/infra/ai/openaicompat`, `./internal/bootstrap`, `./internal/server`, `./internal/architecture`), Admin Vue targeted Vitest (`tests/shared/ai/ai-agent-api.test.ts`, `tests/shared/ai/ai-agent-free-canvas-scenes.test.ts`), and Canvas/Admin typecheck. A larger same-command Canvas Vitest batch hit local Windows/Node memory pressure during this C5 pass; smaller chunks passed for the newly touched audio API/wiring tests.
  ...
  - 2026-06-09 video reference-media upload storage first slice covered by focused Go tests (`./internal/module/ai/video`, `./internal/module/ai/video/transport/canvas`, `./internal/architecture`, `./internal/bootstrap`, `./internal/server`), targeted Canvas Vitest (`src/services/api/video.test.ts`, `tests/shared/canvas-api-boundary.test.ts`), `npm run typecheck`, refreshed route/API artifacts, and runtime doc/governance checks.
  ...
  - 2026-06-09 asset ZIP backend-import text-only guard covered by targeted Canvas Vitest (`src/app/(user)/assets/asset-transfer.test.ts`, `tests/shared/ai-asset-backend-persistence.test.ts`).
...
- Risk:
  ...
  - video advanced params C2-A is only `generate_audio` / `watermark` request pass-through; C2-B keeps reference video/audio generation fail-closed; C2-C only improves upstream error details; the reference-media upload first slice only creates provider-accessible storage metadata. Broader Seedance parameter policy, wiring uploaded references into provider payloads, Admin strategy UI, and live provider success remain separate slices.
  - audio generation has a backend route and frontend wiring, but it is not a full audio product path yet: no `ai_assets` audio type, no dedicated audio task table/history UI, no audio-specific provider reference-media generation wiring, and no live provider-success smoke has been claimed.
  - current-canvas merge import and asset ZIP import fail-closed remain partial slices; browser/manual Cine Make flow, cloud-synced projects, backend asset persistence/ownership remap, media ZIP storage-backed upload/remap, audio asset backend type, explicit DB metadata columns, and broader素材/导入产品化 hardening remain unclaimed until separately tested and documented.
